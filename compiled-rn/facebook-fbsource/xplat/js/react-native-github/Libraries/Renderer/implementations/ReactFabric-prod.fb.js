...
function use(usable) {
  if (null !== usable && "object" === typeof usable) {
    if ("function" === typeof usable.then) {
      ...
      return trackUsedThenable(thenableState, usable, index$26);
    }
    if (
  ...
  if (didBailout)
    for (var child$65 = completedWork.child; null !== child$65; )
      (newChildLanes |= child$65.lanes | child$65.childLanes),
        (subtreeFlags |= child$65.subtreeFlags & 14680064),
        (subtreeFlags |= child$65.flags & 14680064),
  ...
  switch (finishedWork.tag) {
    ...
    case 22:
      flags & 512 &&
        null !== current &&
        safelyDetachRef(current, current.return);
      var isHidden$89 = null !== finishedWork.memoizedState,
        wasHidden$90 = null !== current && null !== current.memoizedState;
      if (finishedWork.mode & 1) {
        var prevOffscreenSubtreeIsHidden = offscreenSubtreeIsHidden,
          prevOffscreenSubtreeWasHidden = offscreenSubtreeWasHidden;
        offscreenSubtreeIsHidden = prevOffscreenSubtreeIsHidden || isHidden$89;
        offscreenSubtreeWasHidden =
          prevOffscreenSubtreeWasHidden || wasHidden$90;
        recursivelyTraverseMutationEffects(root, finishedWork);
        offscreenSubtreeWasHidden = prevOffscreenSubtreeWasHidden;
        offscreenSubtreeIsHidden = prevOffscreenSubtreeIsHidden;
      ...
      switch (didTimeout) {
        ...
        case 4:
          ...
          for (originallyAttemptedLanes = -1; 0 < lanes; )
            (errorRetryLanes$94 = 31 - clz32(lanes)),
              (errorRetryLanes = 1 << errorRetryLanes$94),
              (errorRetryLanes$94 = didTimeout[errorRetryLanes$94]),
              errorRetryLanes$94 > originallyAttemptedLanes &&
                (originallyAttemptedLanes = errorRetryLanes$94),
              (lanes &= ~errorRetryLanes);
              ...
              case 22:
                if (suspenseBoundary.mode & 1) {
                  ...
                  if (null === offscreenQueue) {
                    ...
                    var wakeables$32 = offscreenQueue.wakeables;
                    null === wakeables$32
                      ? (offscreenQueue.wakeables = new Set([wakeable]))
                      : wakeables$32.add(wakeable);
                  }
                  break;
                }
              default:
